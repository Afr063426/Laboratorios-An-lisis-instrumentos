...
\par Una alternativa a este modelo de un factor basado en las tasas esperadas y actuales es el modelo de los tres factores. El cual su explicación se escapa un poco del nivel de este informe.
\section{Resultados}
...
grafico2 <- function(fechas,`Interes acumulado 2` , total, gcapital) {
  ...
  Datosgra<-gather(Datosgra, Obsevaciones, obtenido , names(Datosgra[2]):names(Datosgra[length(Datosgra)]))
  
  ggplot( data =Datosgra, mapping = aes(x = fechas, y = obtenido, group=Obsevaciones, color=Obsevaciones)) + 
    geom_line()+
    scale_color_discrete(labels=c("Ganancia de capital", "Interés acumulado 2", "Total"))+
      labs(
      y = "Monto",
      x = "Tiempo",
      title = "Comportamiento del bono a través tiempo" ) +
...
```{r Grafico del bono 2, fig.show='show'}
...
Fama, E. & Frenc K. (1992). Common risks factors in the returns on stocks and bonds. Recuperado de https://rady.ucsd.edu/faculty/directory/valkanov/pub/classes/mfe/docs/fama_french_jfe_1993.pdf
