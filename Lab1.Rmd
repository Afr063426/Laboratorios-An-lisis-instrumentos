...
\begin{itemize}
\item En la primera columna se encuentra que se está haciendo el cálculo de un sub cupón y cuánto de este se ha ganado, esto es si se tiene un millón en bonos cuanto se gana en cupones
    \[
...
%\par Una alternativa a este modelo de un factor basado en las tasas esperadas y %actuales es el modelo de los tres factores. El cual su explicación se escapa del %objetivo de este informe.
\par Una alternativa a este modelo es el de la aproximación mediante la curva de rendimiento CMT y el Spread de Merril Lynch, este método es empleado en países donde no se cuente con curva soberana, que es una curva de rendimiento que relaciona el rendimiento con el plazo al vencimiento de instrumentos financieros, en cierta medida esto refleja cómo se espera que se comporte el mercado en un período determinado. Costa Rica sí cuenta con una Curva de rendimiento soberana.
...
\par Otro metodo es la aproximación mediante extrapolación polinomial cuadrática. En este caso lo que se hace es usar la extrapolación cuadrática, sin embargo este se emplea cuando se desea determinar los precios, fuera de un intervalo donde ya se conocen dos puntos. En el caso del bono 1 ya se tiene un rendimiento inicial y se tiene uno esperado es decir en cierta forma ya se conocen dos puntos del intervalo y con este es que se calculan los precios.
\section{Resultados}
