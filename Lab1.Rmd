...
#Para bono 2
Funciones que se emplean a lo largo del código
```{r}
...
creacion_de_columnas <-
  function(interes,
    ...
    for (i in 1:(length(fecha) - 1)) {
      ...
      G_capital <-
        c(G_capital,
          (YAS_BOND_PX_2[i + 1] - as.numeric(as.character(
            Datos$`Precio Compra`
          ))) *
            as.numeric(as.character(Datos$Monto)) / 100)
      TOTAL_2 <- c(TOTAL_2, interes2[i] + G_capital[i])
      Retorno_total_2 <-
        c(Retorno_total_2, TOTAL_2[i] / as.numeric(as.character(Datos$Monto)) * 100)
      if (i == (length(fecha) - 1)) {
        TOTAL_2 <- c(TOTAL_2, interes2[i + 1] + G_capital[i + 1])
        Retorno_total_2 <- c(Retorno_total_2
                             ,
...
bono_2<-creacion_de_columnas(interes2_1,interes2_2,YAS_BOND_YLD_2,Gcapital_2 ,ShockF_2)
names(bono_2)<-c("interes2_1","interes2_2","YAS_BOND_YLD_2,Gcapital_2","Gcapital_2","ShockF_2","TOTAL_2","Retorno_total_2")
bono2 <-
  data.frame(
